...
// texture. Restricted to valid-return pixels (finite x), in-place on .reflectivity.

namespace {
using DCloud = pcl::PointCloud<dlio::Point>;
constexpr float kNanX = std::numeric_limits<float>::quiet_NaN();

// Organized W x H cloud; points[row*W + col].reflectivity = refl, x = 1 (valid).
DCloud::Ptr makeOrganized(int W, int H) {
  ...
  for (auto& p : c->points) { p.x = 1.f; p.y = 0.f; p.z = 0.f; p.intensity = 0.f; p.reflectivity = 0.f; }
  return c;
}
}  // namespace
...
TEST(DenoiseOrganizedChannel, KernelLeOneIsNoOp) {
  ...
  for (int i = 0; i < W * H; ++i) { c->points[i].reflectivity = 10.f + i; }  // distinct
  dlio::OdomNode::denoiseOrganizedChannel(c, W, H, 1);   // K=1 -> no-op
  for (int i = 0; i < W * H; ++i) { EXPECT_FLOAT_EQ(c->points[i].reflectivity, 10.f + i); }
...
TEST(DenoiseOrganizedChannel, ReducesHighFrequencyNoise) {
  ...
  for (int r = 0; r < H; ++r) {
    for (int col = 0; col < W; ++col) {
      c->points[r * W + col].reflectivity = base + (((r + col) % 2) ? -amp : amp);
    }
  }
  ...
  for (int r = 1; r < H - 1; ++r) {           // interior only (full 3x3 support)
    for (int col = 1; col < W - 1; ++col) {
      const double d = c->points[r * W + col].reflectivity - base;
      sumsq += d * d; ++cnt;
    }
  }
