...
## Recommended action
- **Do NOT add locks to silence these** as if they were bugs — that adds hot-path
  cost (IMU cb, 100 Hz publish timer) for a non-race on the deployment target.
- **Add a TSan suppressions file** for these known single-group members so future
  TSan runs surface only genuine cross-group races — the way to keep TSan useful
  for actually finding the deg=6 writer. (RESPLE convention:
  `resple/test/tsan_suppressions.txt`.)
- Optional, defensive-for-aarch64 only: give `transformImu` a tiny local-state
  refactor (pass prev-stamp/prev-ang-vel as locals or make them `std::atomic`) and
  publish from locals in `publishPose` — low value on x86.
