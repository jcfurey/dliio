...
2. **Levenberg–Marquardt was replaced by fixed-λ Gauss-Newton with no step control.** `lsq_registration` (LM with ρ-based step acceptance) is dead code on this branch — still *compiled*, never *called* **[PR #1 removes it]**. The replacement in `computeTransformation()` takes every step unconditionally: no cost evaluation, no backtracking, no λ adaptation. GN on a well-conditioned scan-to-map problem usually works, but the entire reason fast_gicp ships LM is the occasional ill-conditioned submap; here that case just NaN-breaks (silently — `hasConverged()` wasn't even set **[PR #1]**). **[OPEN: step acceptance / LM damping; see Part V]**
...
7. **One threshold, three meanings.** `intensity_gradient_threshold_ = 1e-6` is used as (a) a variance floor in intensity², (b) a gradient-magnitude floor in intensity/m, and implicitly (c) against `gradient.norm()` again in `linearize` with different constants (1e-6, 100). Pick units, name constants. **[OPEN — mitigated by channel normalization (all thresholds now in normalized units), but the constants are still unnamed and double-duty; see Part V]**
...
13. **Detached threads everywhere** (`publish_thread`, `metrics_thread`, `debug_thread`, `publish_keyframe_thread`). They capture `this` and outlive nothing in particular: at shutdown a detached `debug()` can race node destruction (use-after-free on publishers/members); `computeMetrics` reads `this->original_scan` (a non-atomic `shared_ptr` member the main thread reassigns) — a data race that's "fine" until it isn't. The idiomatic fix is jthread members joined in the destructor, or just doing this work in the existing callback groups. **[PARTLY FIXED: join-before-respawn pattern + destructor joins all workers and waits for the submap future (shutdown/unload-safe). The benign in-flight races remain: the metrics thread reads `original_scan`/pushes metrics vectors while the main thread reads `.back()`; see Part V]**
...
|---|---|---|---|
| Deskew & observer math | A− | **A−** | Faithful to two strong papers; startup-dt bug fixed **[PR #1]** |
| Registration engine | C | **B** | Honest regularization options, double-precision solve, SO(3) updates, degeneracy gate; still fixed-λ GN without step acceptance |
...
| Concurrency | C− | **B** | Joinable workers, bounded waits, unload-safe destructor; benign metrics races remain |
...
| Testing & CI | F | **C+** | gtest suite + Jazzy CI; no bag-replay regression, no deskew test, no lint |
...
**Code, tractable without data:**
4. **GN step control** (#2) — cost evaluation + step acceptance (or restore LM damping) for ill-conditioned submaps; today a bad step is taken unconditionally and NaN is the only brake.
...
7. **Parameter ranges, read-only flags, live-set callbacks** (#26) — descriptions exist; ranges and `add_on_set_parameters_callback` for the live-tunable gains/weights do not.
8. **Named constants for the gradient thresholds** (#7) — `intensity_gradient_threshold_` still serves as variance floor and magnitude floor with magic bounds (1e-6, 100) in `linearize`.
9. **Metrics-thread data races** (#13) — `original_scan` shared_ptr read and metrics-vector push/back race between the metrics worker and main loop; benign in practice, trivially fixed by computing metrics inline (they're cheap) or snapshotting the pointer.
10. **Deskew unit test + ament_lint** (#28) — `integrateImuInternal` against a synthetic constant-jerk trajectory; lint wiring.
11. **Vendored nanoflann → rosdistro dependency** (#29) — keep the thin adaptor, drop the 2.4k-line header copy.
12. **CMake cleanup** (#30) — `OpenMP::OpenMP_CXX` imported target, stop forcing `CMAKE_BUILD_TYPE`, drop globals, decide whether the static libs are exported or internal.
13. **Misc** (#31) — unused `version` param, dead members in `odom.h` (`imuPose`, `q_final`, `obs_*`, `scan_dt`), `static_assert` on `sizeof(dlio::Point)`, deskew-status introspection topic (#15), per-point covariance precision (#4), boost trimming.
14. **Keyframe pruning validation** (#12) — `maxKeyframes` is implemented but opt-in and untested on long loops; validate revisit behavior before recommending a default.
