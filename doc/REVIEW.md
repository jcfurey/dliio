...
25. **A 1990s terminal dashboard.** `debug()` printf-paints a box-drawing UI with `\033[2J` (clear screen) at scan rate, and `start()` does it again — under `ros2 launch` with two nodes multiplexed this garbles both logs; under systemd it's noise. ROS 2 has `RCLCPP_INFO_THROTTLE` and, for exactly this use-case, `diagnostic_updater` + `rqt_robot_monitor`. At minimum: gate the dashboard behind a parameter, default off, and route warnings through the logger (the map node's `savePCD` talks via `std::cout` exclusively). **[PARTLY FIXED on this branch: `odom/debug/dashboard` parameter (default on for standalone; the composed launch disables it); savePCD logging still via stdout]**
