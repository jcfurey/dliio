...
13. **Detached threads everywhere** (`publish_thread`, `metrics_thread`, `debug_thread`, `publish_keyframe_thread`). They capture `this` and outlive nothing in particular: at shutdown a detached `debug()` can race node destruction (use-after-free on publishers/members); `computeMetrics` reads `this->original_scan` (a non-atomic `shared_ptr` member the main thread reassigns) — a data race that's "fine" until it isn't. The idiomatic fix is jthread members joined in the destructor, or just doing this work in the existing callback groups. **[FIXED on this branch: join-before-respawn pattern + destructor joins all workers and waits for the submap future]**
